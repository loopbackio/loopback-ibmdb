...
const debug = require('debug')('loopback:ibmdb');

module.exports = function(IBMDB) {
  IBMDB.prototype.showFields = function(model, cb) {
    const self = this;
    const sql = 'SELECT COLNAME AS NAME, TYPENAME AS DATATYPE, ' +
  ...
  IBMDB.prototype.showIndexes = function(model, cb) {
    const self = this;
    const sql = 'SELECT TABNAME, TABSCHEMA, INDNAME, ' +
  ...
  IBMDB.prototype.dropIndexes = function(model, actualIndexes) {
    ...
    if (actualIndexes) {
      actualIndexes.forEach(function(i) {
        const isPrimaryIndex = i.UNIQUERULE === 'P';
        let stmt;
        if (isPrimaryIndex) {
  ...
  IBMDB.prototype.alterTable = function(model, actualFields, actualIndexes,
    ...
    let operations = self.getAddModifyColumns(model, actualFields);
