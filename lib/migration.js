...
module.exports = function(IBMDB) {
  ...
  IBMDB.prototype.addIndexes = function(model, actualIndexes, cb) {
    var ai = {};
    var self = this;
  ...
  IBMDB.prototype.changeOrAddColumns = function(model, actualFields, cb) {
    var self = this;
    var operations = [];
    var m = this.getModelDefinition(model);
    var propNames = Object.keys(m.properties).filter(function(name) {
  ...
  callback, checkOnly) {
    ...
    var sql = [];
    var tasks = [];
    self.changeOrAddColumns(model, actualFields, function(res) {
      sql = sql.concat(res);
    });
    self.addIndexes(model, actualIndexes, function(res) {
      sql = sql.concat(res);
    });
